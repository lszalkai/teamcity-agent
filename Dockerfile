...
ARG JDK_25_URL=https://corretto.aws/downloads/latest/amazon-corretto-25-x64-linux-jdk.tar.gz
...
RUN mkdir -p /opt/java/jdk-8  && curl -fsSL $JDK_8_URL | tar -xvzf - -C /opt/java/jdk-8 --strip-components=1  && \
    ...
    mkdir /opt/java/jdk-25 && curl -fsSL $JDK_25_URL | tar -xvzf - -C /opt/java/jdk-25 --strip-components=1 && \
    rm -rf *.tar.gz
...
ENV JDK_25_64=/opt/java/jdk-25

ENV JRE_HOME=/opt/java/jdk-25
